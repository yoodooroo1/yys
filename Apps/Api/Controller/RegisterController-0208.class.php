...
class RegisterController extends Controller {
    ...
    public function index(){
		...
		if($paytype == 3){  //如果为余额支付
			...
			if($consume_money < $need_money){
				...
				$params['periods'] = date('Y-m',mktime());;
	...
	protected function confirm($member_id=''){
		//$member_id = I('member_id');   
		$m = M('vip_orders'); 
		$info =$m->where(array('member_id'=>$member_id))->find();       
		...
		if($m->where(array('member_id'=>$member_id))->save($data)){	
			/*更新会员等级和推荐人*/ 
			$member = M('members');
			...
			if(!empty($parents['p1'])){
				...
				if($money1>0){
					...
					$params1['description'] = '发展'.$profit_share['level_name'].'会员奖励（直接）';
					...
					$params2['description'] = '发展'.$profit_share['level_name'].'会员奖励（直接）';
			...
			if(!empty($parents['p2'])){
				...
				if($money2>0){
					...
					$params3['description'] ='发展'.$profit_share['level_name'].'会员奖励（间接）';
					...
					$params4['description'] = '发展'.$profit_share['level_name'].'会员奖励（间接）';
			...
			if(!empty($parents['p3'])){
				$money3 =  $packageinfo['pv']* $packageinfo['percent3']/100;
				if($money3>0){ 
					$params5 = array();
					$params5['member_id'] = $parents['p3'];
					$params5['type'] = '2';      //消费钱包
					$params5['prices'] = $money3*$sconfig['award_shop_percent']/100;
					$params5['periods'] = $periods;
					$params5['description'] = '发展'.$profit_share['level_name'].'会员奖励（三维）';
					$params5['addtime'] = mktime();
					$wallet_record->add($params5);  
					$wallet->where(array("member_id"=>$parents['p3']))->setInc('consume_money',$money3*$sconfig['award_shop_percent']/100);
					
					$params6 = array(); 
					$params6['member_id'] = $parents['p3'];
					$params6['type'] = '3';      //可提现钱包（现金钱包）
					$params6['prices'] = $money3*$sconfig['award_cash_percent']/100;
					$params6['periods'] = $periods;
					$params6['description'] = '发展'.$profit_share['level_name'].'会员奖励（三维）';
					$params6['addtime'] = mktime();
					$wallet_record->add($params6);   
					$wallet->where(array("member_id"=>$parents['p3']))->setInc('cash_money',$money3*$sconfig['award_cash_percent']/100);
				}   
			}
