...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        share_diyUpload:function( opt, serverCallBack ) {
			...
			var webUploader = share_getUploader( opt );
			...
			webUploader.on('fileQueued', function( file ) {
				share_createBox( $fileInput, file ,webUploader);
			...
			webUploader.on('uploadProgress',function( file, percentage  ){
				...
				share_showDiyProgress( percentage.toFixed(2), $diyBar);
			...
			webUploader.on('uploadError',function( file, reason ){
				var $fileBox = $('#fileBox_'+file.id);
				var $diyBar = $fileBox.find('.diyBar');	
				share_showDiyProgress( 0, $diyBar , '上传失败!' );
	...
	function getOption(objId) {
		...
		*/
		return {
			...
			method:"POST",
			//服务器地址;
			server:"http://www.xunxin.biz/admin.php/Service/Goods/goods_insert",
	...
	//实例化Web Uploader
	function share_getUploader( opt ) {
 
		return new WebUploader.Uploader( opt );;
	}
	
	//操作进度条;
	function share_showDiyProgress( progress, $diyBar, text ) {
	...
	//取消事件;	
	function share_removeLi ( $li ,file_id ,webUploader) {
	...
	//创建文件操作div;	
	function share_createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			$cancelButton.bind('click',function(){
				...
				$.each( fileArr ,function( i, v ){
					share_removeLi( $('#fileBox_'+v.id), v.id, webUploader );
				});
			});
		...
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
				    ...
				    <input type="number_'+ file_id +'" class="number_add" name="share_imgsort[]"  placeholder="此图序号(阿拉伯数字)"/>\
					<div class="diyText"><textarea placeholder="此图描述"></textarea></div> \
				</li>';  
		...
		var $diyCancel = $fileBox.children('.diyCancel').one('click',function(){
			share_removeLi( $(this).parent('li'), file_id, webUploader );	
		});
